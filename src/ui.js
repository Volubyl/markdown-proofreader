...
const filterReplacement = (replacements) => replacements
  .map((item) => item.value)
  .map((item) => item.trim())
  .filter((item) => item);
...
const formatMessage = (message) => `${chalk.red(String.fromCharCode(10007))} ${chalk.bold(message)}`;

const formatSentence = (sentence) => `${chalk.bold(String.fromCharCode(8227))} ${chalk.bold(
  'Sentence:',
)} ${sentence}`;

const formatFilePath = (filepath) => chalk.bold.magentaBright(`${String.fromCharCode(8608)} File: ${filepath}`);
...
const formatSuccessMessage = (successMessage) => chalk.green(`${String.fromCharCode(10004)} ${successMessage}`);

const makeOneReportDisplayable = (report) => report.reduce((prev, current) => {
  if (!prev) {
...
const displaySuccessMessage = () => log(formatSuccessMessage('Allright! No mistake found, My Capitain'));
